...
/**
 * Servlet implementation class PermDelete
 */
public class PermDelete extends HttpServlet {
	...
	/*	request.setCharacterEncoding("UTF-8");
		...
		List<Role> roleList=us.getRolesByPermission(perm);
		if(roleList.size()>0)
		{
			String msg;
			msg="Permission ["+perm.getPermissionName()+"] belongs to role:";
			for(Role role:roleList)
			{
				msg=msg+role.getRoleName()+" ";
			}
			jo.put("errorMsg",msg);
			response.getWriter().println(jo.toString());
			return;
		}
		us.deletePermission(perm);
